...
enum myblake2s_constant
{
	myblake2s_BLOCKBYTES = 64,
	myblake2s_OUTBYTES   = 32,
	myblake2s_KEYBYTES   = 32,
	myblake2s_SALTBYTES  = 8,
	myblake2s_PERSONALBYTES = 8
};

#pragma pack(push, 1)
typedef struct __myblake2s_param
{
	...
	// uint8_t  reserved[0];
	uint8_t  salt[myblake2s_SALTBYTES]; // 24
	uint8_t  personal[myblake2s_PERSONALBYTES];  // 32
} myblake2s_param;

ALIGN( 64 ) typedef struct __myblake2s_state
{
	uint32_t h[8];
	uint32_t t[2];
	uint32_t f[2];
	uint8_t  buf[2 * myblake2s_BLOCKBYTES];
	size_t   buflen;
	uint8_t  last_node;
} myblake2s_state;
#pragma pack(pop)
...
extern "C" {
	...
	#define myblake2s_salt32(out, in, inlen, key32) myblake2s(out, in, key32, 32, inlen, 32) /* neoscrypt */
	#define myblake2s_simple(out, in, inlen) myblake2s(out, in, NULL, 32, inlen, 0)
