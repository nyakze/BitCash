        ...
        while (file.good()) {
            ...
            if (key.IsValid()) {
                ...
                if (pwallet->HaveKey(keyid)) {
                    LogPrintf("Skipping import of %s (key already present)\n", EncodeDestination(keyid,pubkey));
                    continue;
                }
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to bitcashd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/bitcash/bitcash/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Bitcash %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see bitcashd log for details) and could "
