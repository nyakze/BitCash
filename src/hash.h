...
#ifndef BITCASH_HASH_H
#define BITCASH_HASH_H
...
/** A hasher class for Bitcash's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
public:
...
/** A hasher class for Bitcash's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
public:
...
inline uint256 HashX25X(const T1 pbegin, const T1 pend)
{
    ...
    myblake2s_simple((uint8_t*)&hash[24], static_cast<void*>(&hash[0]), 64 * 24);

    return hash[24].trim256();
}


#endif // BITCASH_HASH_H
